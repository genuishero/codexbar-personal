...
```sh
python3 scripts/sync_version_metadata.py
python3 scripts/generate_update_feed.py release-feed/stable.json release-feed/stable.json
```

`sync_version_metadata.py` 会把 `VERSION` 同步到：

- Xcode `MARKETING_VERSION`
- `release-feed/stable.json` 的 `release.version`
- `releaseNotesURL` / `downloadPageURL`
- 每个 artifact 的 `downloadURL`

如果某个 artifact 额外提供了 `localPath`，`generate_update_feed.py` 会在本地文件存在时自动计算 `sha256` 并写回输出。

每次改版本后，必须再跑一次一致性校验：

```sh
python3 scripts/sync_version_metadata.py --check
